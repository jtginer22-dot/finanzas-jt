...
- **Desayuno con Trini** → Categoría `Restaurantes` (es una comida planeada, aunque sea la primera del día — no es `Snacks`). Etiqueta `Polola` opcional.
- **Notaría por promesa de compraventa de una propiedad** → Categoría `Vivienda` (no se crea categoría nueva por ser puntual). Como probablemente vengan más costos de la misma compra (escritura, inscripción, corretaje), conviene una etiqueta reutilizable tipo `Compra depto`/`Compra casa` para poder sumar el costo total del trámite después.
- **Farmacia** → NO es automáticamente `Salud` solo por ser farmacia — depende de qué se compró (medicamento/insumo médico → `Salud`; shampoo, maquillaje, snack comprado ahí → su categoría real, `Ropa y personal`/`Snacks`/etc.). Ej: anticonceptivos de Trini pagados por José → `Salud` + etiqueta `Polola` (es un medicamento; la etiqueta distingue que es gasto de/para ella).
