...
**Fix de UX**: `updatePendSortDirLabels()` — el texto de "Dirección" ahora cambia según la clave activa ("↑ Menor monto primero" solo cuando Ordenar por = Monto; "↑ Más antiguo primero" cuando = Fecha; "↑ A → Z" cuando = Comercio), para que no se pueda mal-interpretar cuál campo se está ordenando.
